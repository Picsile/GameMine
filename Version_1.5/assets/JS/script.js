...
const World = {
    ...
    generateWorld() {
        ...
        function generateFluid() {
            ...
            for (let y = this2.startheight; y < this2.heightArray; y++) {

                if (y < this2.heightArray - 10) {
                    for (let x = 0; x < this2.widthArray; x++) {

                        if (this2.map[y][x] != undefined && this2.map[y-1][x] != 14) {

                            if (widthFluid > 5 && widthFluid < 20) {

                                if (Math.floor(Math.random() * 80) == 0) {

                                    for (let height = 3; height < 10; height++) {

                                        if (this2.map[y + height][x - Math.floor(widthFluid / 2)] != undefined) {
                                            heightFluid = height;
                                        } else break;
                                    }

                                    if (heightFluid > 0) {
                                        heightFluid = Math.min(Math.floor(widthFluid/2) + 1, heightFluid);
                                        const masFluid = [[]];
                                        const typeFluid = Math.floor(Math.random() * fluidList.length);

                                        // Создаём шаблон
                                        for (let n = 0; n < heightFluid; n++) {                                
                                            (n < heightFluid - 1) ? masFluid.push([]) : null;

                                            for (let m = 0; m < widthFluid; m++) {
                                                const indentation = n + Math.floor(Math.random() * 2);
                                                
                                                // Создание бортиков
                                                if (m > indentation && m < widthFluid - indentation - 1) {
                                                    masFluid[n][m] = 4;
                                                    
                                                    (n < heightFluid - 1) ? masFluid[n + 1][m] = fluid[fluidList[typeFluid]]["blockNearby"] : null;
                                                } else if (m == indentation || m == widthFluid - 1 - indentation) {
                                                    masFluid[n][m] = fluid[fluidList[typeFluid]]["blockNearby"];
                                                } else masFluid[n][m] = null;
                                            }
                                        }

                                        spawn(y, x - widthFluid, [masFluid], 0);
                                    }
...
const blocks = {
    ...
    "BlockSand": {
        "src": "assets/img/textures/Блок песок.jpg",
        "collision": true,
        "flowability": true,
        "breakingSpeed": 3.5,
        "typeOfTool": "Shovel",
...
const fluid = {
    "Water": {
        "src": "assets/img/textures/Water.jpg",
        "damage": 0,
        "blockNearby": 17
    },
    "Lava": {
        "src": "assets/img/textures/Lava.jpg",
        "damage": 30,
        "blockNearby": 2
    }
}
...
// Список проиндексированых жидкостей
const fluidList = ["Water", "Lava"];
...
function generateMap(width = 100, height = 100) {
    ...
    World.generateWorld()[7](); // Деревья
    World.generateWorld()[8](); // Жидкости
